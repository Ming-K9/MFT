...
class FlatArguments:
    ...
    subnet_mode: Optional[str] = None
    subnet_type: Optional[str] = None
    sparsity_attn: Optional[float] = None
    sparsity_mlp: Optional[float] = None
    threshold_attn: Optional[float] = None
    threshold_mlp: Optional[float] = None
    masked_layers: Optional[str] = None
    only_save: bool = field(
        default=False
    )
    calculate_tokens: bool = field(
        default=False
    )
...
def main(args: FlatArguments):
    ...
    if args.config_name:
        ...
        if args.subnet_type == "local":
            ...
            config = AutoConfig.from_pretrained(
                ...
                subnet_mode=args.subnet_mode,
                subnet_type=args.subnet_type,
                threshold_attn=args.threshold_attn,
                threshold_mlp=args.threshold_mlp,
            )
        else:
            print("Using original model config.")
            config = AutoConfig.from_pretrained(
                args.model_name_or_path,
                revision=args.model_revision,
                trust_remote_code=args.trust_remote_code,
    ...
    if args.use_lora:
        with GatheredParameters(model.parameters(), modifier_rank=0):
            tokenizer.save_pretrained(os.path.join(args.output_dir, "init"))
            model.save_pretrained(os.path.join(args.output_dir, "init"),safe_serialization=False)
        if args.only_save:
            return
    ...
    if args.subnet_mode != None:
        for name, param in model.named_parameters():
            if "scores" not in name:
                param.requires_grad = False
    for name, param in model.named_parameters():
        if param.requires_grad == True:
            accelerator.print(f"Trainable parameter: {name}")
    ...
    if args.calculate_tokens:
        total_tokens = sum(len(example["input_ids"]) for example in train_dataset)
        logger.info(f"Total tokens for training: {total_tokens}")
        return
    ...
    accelerator.print("\nChecking parameters in optimizer:")
    opt_param_ids = set()
    ...
    for name, param in model.named_parameters():
        if id(param) in opt_param_ids:
            accelerator.print(f"Parameter in optimizer: {name}")
    ...
    if checkpointing_steps is not None and str(checkpointing_steps).lower() == "epoch/3":
        checkpointing_steps = int(num_update_steps_per_epoch / 3)
    elif checkpointing_steps is not None and str(checkpointing_steps).lower() == "epoch/2":
        checkpointing_steps = int(num_update_steps_per_epoch / 2)
    elif checkpointing_steps is not None and str(checkpointing_steps).lower() != "epoch":
            ...
            if accelerator.sync_gradients:
                ...
                if isinstance(checkpointing_steps, int):
                    if completed_steps % checkpointing_steps == 0:
                        ...
                        model_save_dir = f"model_step_{completed_steps}"
                        if args.output_dir is not None:
                            model_save_dir = os.path.join(args.output_dir, model_save_dir)
    ...
    if args.output_dir is not None:
        if checkpointing_steps is not None and (str(checkpointing_steps).lower() == "epoch/3" or str(checkpointing_steps).lower() == "epoch/2"):
            model_save_dir = f"model_step_{args.max_train_steps}"
            if args.output_dir is not None:
                model_save_dir = os.path.join(args.output_dir, model_save_dir)
            os.makedirs(model_save_dir, exist_ok=True)
            save_with_accelerate(
